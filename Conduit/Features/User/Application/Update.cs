...
namespace Conduit.Features.User.Application
{
    public class Update
    {
        ...
        public async Task<AunthenticateUserEnvelop> UpdateUserFileInfo(UserUpdateData data)
        {
            ...
            await Save();
            return new AunthenticateUserEnvelop(new AunthenticatedUser { email = user.Email, token = _jvtService.CreateToken(user.Username, user.Role), role = user.Role, username = user.Username, bio = user.Bio, image = user.Image });
        }
